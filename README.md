...
A single-property site for a pair of new townhouses in the Clinton Hill Historic
District, Brooklyn, by StudioSC Architecture.

Built against the teardown of quadplex80.com in `docs/capture/` — the structure
and interaction design are modelled on it; all content is our own.
...
**The slide machine** (`src/lib/slides.ts`). Five full-screen slides — the hero
plus four floors. `body { overflow: hidden }`, no document scroll, wheel
captured through `Lethargy(8, 50)` with a 50ms trailing debounce. One gesture
advances exactly one slide, input is locked for the duration of the transition,
and the clamp at either end is hard: no wrap, no rubber-band.
...
1. **The hero is 735×633.** It is the only exterior rendering that exists and it
   is visibly soft upscaled to a 1440px stage. High-resolution originals have
   been requested from StudioSC. This is the longest-lead item in the project.
2. **The scrim colour is wrong for this building.** `--c-scrim: #3c6278` is the
   reference's slate blue, chosen to sit under a sky. Against warm brick and
   plaster it reads cold, and the slide counter is barely legible over the
   bright lower-right of the hero. Fix this in the re-skin — pick a warm neutral
   out of the interior deck's oak and stone, per `REBUILD.md` §4.
3. **Two slides use stand-in imagery.** No living-room rendering exists for the
   second floor and no bedroom renderings for the third; `src/data/slides.ts`
   marks each with a `placeholder` note.
4. **Chrome is scaffolding.** The slide counter and "Scroll" hint stand in for
   the pip rail and slide-down button in Phase 2.
5. **Fonts are substitutes.** Instrument Serif + Inter Tight until Teodor and
...
the `.uline` underline pair. Waiting on `sections.md` for verbatim copy.
